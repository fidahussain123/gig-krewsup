      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
        <View className="flex-row items-center justify-between bg-white p-6">
          <View className="flex-row items-center gap-3">
            ...
            <View>
              <Text className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Welcome back</Text>
              <Text className="text-xl font-extrabold text-slate-900">{user?.name || 'Organizer'}</Text>
            </View>
          </View>
          <Pressable className="h-11 w-11 items-center justify-center rounded-full bg-white ring-1 ring-slate-100">
        ...
        <View className="p-6 pt-2">
          <View className="flex-row flex-wrap gap-4">
            ...
            >
              <View>
                <Text className="text-xs font-bold uppercase tracking-widest text-white/80">Pending To Pay</Text>
                <Text className="mt-1 text-3xl font-extrabold text-white">₹{stats.pendingToPay.toLocaleString()}</Text>
              </View>
              <View className="rounded-2xl bg-white/20 p-4">
                <Icon name="account_balance_wallet" className="text-white text-4xl" />
              </View>
            </Pressable>
            ...
            <View className="flex-1 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
              ...
              <Text className="text-3xl font-extrabold text-slate-900">{stats.activeGigs}</Text>
              <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Active Events</Text>
            </View>
            ...
            <View className="flex-1 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100 border-l-4 border-accent">
              ...
              <Text className="text-3xl font-extrabold text-slate-900">{stats.applicants}</Text>
              <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Applicants</Text>
            </View>
          </View>
        </View>
        ...
        <View className="mt-2">
          <View className="flex-row items-center justify-between px-6 pb-4">
            <Text className="text-xl font-extrabold text-slate-900">Your Events</Text>
            <Pressable onPress={() => router.push('/organizer/create-event')}>
              <Text className="text-sm font-bold text-primary">+ Create New</Text>
            </Pressable>
          </View>
          ...
          {isLoading ? (
            ...
            <View className="mx-6 mb-6 rounded-3xl bg-white p-8 items-center shadow-sm ring-1 ring-slate-100">
              ...
              <Text className="text-lg font-extrabold text-slate-900 mb-2">No Events Yet</Text>
                  ...
                  >
                    <View className="relative aspect-video w-full overflow-hidden rounded-2xl bg-slate-100">
                      ...
                      <View className="absolute right-3 top-3 rounded-2xl bg-white/95 px-3 py-2 items-center">
                        <Text className="text-[10px] font-bold uppercase text-primary leading-none">{date.month}</Text>
                        <Text className="text-lg font-extrabold text-slate-900 leading-none mt-1">{date.day}</Text>
                      </View>
                      <View className={`absolute left-3 top-3 rounded-full px-3 py-1 ${
                        event.status === 'published' ? 'bg-green-500' : 'bg-slate-200'
                      }`}>
                        <Text className={`text-[10px] font-bold uppercase tracking-wider ${
                          event.status === 'published' ? 'text-white' : 'text-slate-600'
                        }`}>
                          {event.status}
                        </Text>
                      </View>
                    ...
                    <View className="px-2 py-3">
                      <Text className="text-lg font-extrabold text-slate-900">{event.title}</Text>
                      <View className="mt-1 flex-row items-center gap-1">
                        <Icon name="location_on" className="text-slate-400 text-sm" />
                        <Text className="text-xs font-bold text-slate-400">{event.location || 'Location TBD'}</Text>
                      </View>
                    </View>
                  </Pressable>
                );
        ...
        <View className="mt-2 px-6 pb-10">
          ...
          >
            ...
            <Text className="text-lg font-extrabold text-white">Create New Event</Text>
          </Pressable>
        </View>
      </ScrollView>
    </View>
